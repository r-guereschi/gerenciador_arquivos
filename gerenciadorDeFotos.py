...
def rename_photos(main_folder, new_prefix):
    photo_files = sorted(
        [f for f in os.listdir(main_folder) if f.endswith(("jpg", "jpeg", "png"))]
    )
    renamed_files = []
    ...
    for photo in photo_files:
        ...
        if match:
            ...
            os.rename(
                os.path.join(main_folder, photo), os.path.join(main_folder, new_name)
            )
...
def create_subfolders(main_folder, photos_per_folder):
    ...
    for i in range(num_subfolders):
        ...
        for photo in photo_files[start_idx:end_idx]:
            shutil.move(
                os.path.join(main_folder, photo), os.path.join(subfolder_path, photo)
            )
...
def on_rename():
    ...
    new_prefix = simpledialog.askstring(
        "Input", "Digite o novo prefixo para os arquivos de foto:"
    )
    if not new_prefix:
        return
    ...
    if renamed_files:
        messagebox.showinfo(
            "Sucesso", f"Arquivos renomeados com o prefixo '{new_prefix}'."
        )
    else:
...
def on_divide():
    ...
    photos_per_folder = simpledialog.askinteger(
        "Input", "Quantas fotos você deseja por subpasta?"
    )
...
def main():
    ...
    tk.Button(
        root,
        text="Dividir Fotos em Subpastas",
        image=oval_image,
        compound=tk.CENTER,
        command=on_divide,
        **button_style,
    ...
    tk.Button(
        root,
        text="Remover Subpastas e Mover Fotos",
        image=oval_image,
        compound=tk.CENTER,
        command=on_move,
        **button_style,
